...
#Wykres gęstości zarobków- widac, ze nie jest to rozkład normalny- test Shapiro-wilka
#p-value spr
...
```{r}
ggplot(dane_dojazd, aes(x = commute_distance, y = liczba, fill = purchased_bike)) +
...
#ilość osób zamęznych vs wiek- 2 metoda
```{r}
newdata <- c()
ageGroups <- c(10, 20, 30, 40, 50, 60, 70, 80)
for (ind in ageGroups){  newdata <- c(newdata, count(subset(Credit, Age >= ind & Age <= ind + 10 & Married == "No"))$n)
}
ggplot(data.frame(newdata), aes(x=ageGroups, y=newdata)) + geom_line()
```
