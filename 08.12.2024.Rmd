...
ggplot(dane_zliczone, aes(x = marital_status, y = liczba, fill = purchased_bike)) +
  ...
  scale_fill_manual(values = c("Yes" = "pink", "No" = "white"), name = "Czy posiada rower?")
```
...
ggplot(df_pie, aes(x = "", y = percentage, fill = purchased_bike)) +
  ...
  labs(title = "Procent osób posiadających rower w podziale na płeć", fill = "Czy posiada rower?") +
...
ggplot(sklep_rowerowy, aes(x = factor(children), fill = purchased_bike)) +
  ...
  labs(title = "Czy posiadanie roweru zależy od liczby dzieci?",
       x = "Liczba dzieci", y = "Liczba osób") +
  scale_fill_manual(values = c("Yes" = "#00BFC4", "No" = "#F8766D"), 
                    name = "Czy posiada rower?") +
...
ggplot(sklep_rowerowy, aes(x = Occupation, fill = Purchased.Bike)) +
...
```{r}
  ...
  group_by(home_owner, purchased_bike) %>%
  summarise(count = n()) %>%
  mutate(percentage = round(count / sum(count) * 100, 1))

ggplot(df_pie, aes(x = "", y = percentage, fill = Purchased.Bike)) +
  geom_bar(stat = "identity", width = 1, color = "white") +
  coord_polar("y", start = 0) +
  facet_wrap(~Home.Owner) +
  geom_text(aes(label = paste0(percentage, "%")), position = position_stack(vjust = 0.5), color = "white") +
  labs(title = "Czy kupił rower vs Posiadanie domu", fill = "Czy kupił rower?") +
  theme_void()
```
...
ggplot(sklep_rowerowy, aes(x = factor(cars), fill = purchased_bike)) +
  geom_bar(position = "fill") +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(values = c("Yes" = "darkgreen", "No" = "purple")) +  # Zmiana kolorów
  labs(title = "Czy kupił rower w zależności od liczby samochodów", 
       x = "Liczba samochodów", 
       y = "Procent", 
       fill = "Czy kupił rower?") +
  theme_minimal()
