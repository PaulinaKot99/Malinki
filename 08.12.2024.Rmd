...
ggplot(sklep_rowerowy, aes(x=`commute_distance`,fill=`purchased_bike`)) + 
  geom_histogram(binwidth=30, color="blue") +
  labs(title="Zależność odległości do pracy z posiadaniem roweru", x="odleglość dojazdu", y="Posiadanie roweru") +
  theme_ipsum()
```
# Utwórz wykres (histogram):
```{r}
p <- ggplot(sklep_rowerowy, aes(x=Commute_Distance,
 y=Cars,
 ...
 ylab("Cars") +
 scale_color_discrete(name = "Czy posiada rower?") +
 ylim(0,10) +
 ggtitle("możliwość dojazdu rowerem") +
 theme_light()
plotly::ggplotly(p)
